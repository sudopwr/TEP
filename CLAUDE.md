...
for. It runs on one machine, bound to `127.0.0.1`, for one user. A payout arrives as USD
on a prop firm platform and moves through a processor, crypto wallets and an exchange
before landing as INR in a bank, each hop with its own fee, rate, reference and document.
That lived in a spreadsheet, where copy-paste destroyed four amounts and two fees.
...
INR rate -> gross proceeds, the exchange's schedule for exchange fee and GST, TDS from
the statement, net INR out. Where 0.5% and 18% live. **UC4 — AttachDocument.** Hashes,
stores, dedupes, links: re-uploading links the existing document rather than duplicating
it. **UC5 — GetPayoutTrail.** The tree, with amounts, fees and documents at each node.
**UC6 — GetSettlement.** Gross proceeds, fees by type, net credited. **UC7 —
GetAccountBalances.** Per account per currency, derived never stored. **UC8 —
RunDataQualityChecks.** Flagged rows with reasons. **UC9 — SearchDocuments.** FTS5 over
filename and text. **UC10 — GenerateFinancialYearReport.** A date range: credited / TDS
/ fees by company. **UC11 — SignIn.** Verifies the hash, opens a session, reports the
must-change flag; §5a's one message either way, and a weaker credential re-hashed
*without* clearing the flag. **UC12 — AuthenticateSession.** Session ID to user, or
reject; revoked wins over expired, and it extends past half-life. **UC13 —
ChangeCredentials.** The current password *first*, so it is no "is that username taken?"
oracle; the policy against the *new* username; must-change cleared only if the password
changed; every other session revoked. **UC14 — SignOut.** Revokes server-side,
idempotent, identical for a forged ID as a spent one. **UC15, UC16 — DeletePayout,
DeleteTransaction.** F18 and F20: count first, then one atomic delete — a payout takes
its legs, a leg takes the legs below it. **UC17, UC18 — EditTransaction,
DeleteDocument.** F21: UC2's checks on a rebuilt row, kind, parent and payout carried.
F22: the row, its links, then the file. **UC19, UC20, UC21 — LinkDocument,
DetachDocument, ListDocumentsFor.** F23: attach one already on file, take one off, and
read what is attached to a payout. **Not numbered**, because §3 assumed they existed:
...
- Errors are typed classes in `core/domain/errors.ts`, never bare strings, each message
  written for the reader — the browser passes it through.
...
- **Money as scaled integers, scale per currency**: one fixed scale breaks INR or USDT,
  ...
  **Status and totals are derived**; a stored status drifts the first time a row is
  ...
  integer minor units, superseding "render the server's `amount` string" — a `scale`
  prop and the currency table keep that honesty. `Intl.NumberFormat` takes an exact
  decimal *string*, so `2^53 + 1` paise renders digit for digit and rupees group
  Indian-style; an unknown currency **throws**, guessing 2 for an 8-decimal token being
  a plausible balance.
...
- **One composition root per process**: `container.ts` on the server (the F12 CLI wired
  ...
  `transactions/` would leave neither readable. **The palette is a lint rule**:
...
- **`/api/accounts` and `/api/accounts/balances` answer two questions**: a balance is
  ...
  **set**, sorted: SQLite reads it back ordered and a fake does not.
...
- **A missing choice is added from inside the dropdown** (`SelectWithCreate`), whose
  ...
  box; assert with `find`, since a closing modal still holds `aria-hidden`.
- **What deletes, and what refuses.** A **payout** deletes for real (F18), not into a
  ...
  **fees**: TDS came off a statement, so §7's checks report a mismatch rather than an
  edit overwriting evidence with arithmetic. **Removing a document is not deleting it**
  (F23): a link is a relationship, so breaking one leaves the file on record to be
  attached again — here, or to the leg it belonged to. A **document** is a thing, not a
  relationship (F6): deleting one takes every attachment with it, file *after* row,
  since a row without bytes answers 422 while bytes without a row are litter. The payout
  delete alone **removes** cache entries instead of invalidating them — a deleted trail
  404s.
